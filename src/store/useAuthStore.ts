...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      logout: async () => {
        // Trigger server-side logout requests in the background first so they use current credentials
        authApi.logout().catch(err => console.warn("Background authApi logout warning:", err));
        supabase.auth.signOut().catch(err => console.warn("Background Supabase signOut warning:", err));

        // Instantly clear local session state for instantaneous UI feedback
        localStorage.removeItem('auth_token');
        localStorage.removeItem('refresh_token');
        set({ 
          user: null, 
